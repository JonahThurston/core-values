...
export class ValuesManagerService {
  ...
  deleteBucket(idToDelete: number) {
    ...
    this.userBuckets.update((oldBuckets) =>
      oldBuckets.filter((bucket) => bucket.id !== idToDelete)
    );
  ...
  addToBucket(valToAdd: CoreValue, givenBucket: ValueBucket) {
    const alreadyExistsAt = givenBucket.values.findIndex(
      (value) => value.id === valToAdd.id
    );
    if (alreadyExistsAt != -1) {
      return;
    }

    const bucketIndex = this.userBuckets().findIndex(
      (bucket) => bucket.id === givenBucket.id
    );
  ...
  trashValFromBucket(valToDelete: CoreValue, bucket: ValueBucket) {
    const foundAt = bucket.values.findIndex(
      (value) => value.id === valToDelete.id
    );
    if (foundAt === -1) {
      console.error(
        `Tried to delete val from bucket it was not it: ${valToDelete.id}`
      );
      return;
    }
  ...
  switchBuckets(
    valToSwitch: CoreValue,
    oldBucket: ValueBucket,
    newBucket: ValueBucket
  ) {
    const oldBucketIndex = oldBucket.values.findIndex(
      (value) => value.id === valToSwitch.id
    );
    if (oldBucketIndex === -1) {
      console.warn(
        `tried to bucket switch a value that couldn't be found in the original bucket. id: ${valToSwitch.id}`
      );
      return;
    }
    const newBucketIndex = newBucket.values.findIndex(
      (value) => value.id === valToSwitch.id
    );
    if (newBucketIndex != -1) {
      console.warn(
        `tried to bucket switch a value that was already in the new bucket. id: ${valToSwitch.id}`
      );
      return;
    }

    const oldBIndex = this.userBuckets().findIndex(
      (b) => b.id === oldBucket.id
    );
    const newBIndex = this.userBuckets().findIndex(
      (b) => b.id === newBucket.id
    );
